...
def combine_data_list(data_dir):
    ...
    with open(os.path.join(data_dir, 'all_data_list.txt'), 'w') as f:
        ...
        s1 = '整理前的数据：neg数量：{} pos数量：{} part数量:{} 基数:{}'.format(len(neg), len(pos), len(part),
                                                                            base_num)
        print(s1)
        ...
        s2 = '整理后的数据：neg数量：{} pos数量：{} part数量:{} '.format(len(neg_keep), len(pos_keep),
                                                                       len(part_keep))
        print(s2)
...
def save_hard_example(data_path, save_size):
    ...
    """
    # 获取原数据集中的标注数据
    filename = os.path.join(data_path, 'my_wider_face_train.txt')
