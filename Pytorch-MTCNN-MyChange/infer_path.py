...
# 获取P模型
pnet = torch.jit.load(os.path.join(args.model_path, 'my-PNet.pth'))
pnet.to(device)
softmax_p = torch.nn.Softmax(dim=0)
pnet.eval()

# 获取R模型
rnet = torch.jit.load(os.path.join(args.model_path, 'my-RNet.pth'))
rnet.to(device)
softmax_r = torch.nn.Softmax(dim=-1)
rnet.eval()

# 获取R模型
onet = torch.jit.load(os.path.join(args.model_path, 'my-ONet.pth'))
onet.to(device)
...
def predict_onet(infer_data):
    ...
    infer_data = torch.tensor(infer_data, dtype=torch.float32, device=device)
    # 执行预测
    cls_prob, bbox_pred = onet(infer_data)
...
def detect_onet(im, dets, thresh):
    ...
    for i in range(num_boxes):
        ...
        img = (img - 127.5) / 128
        cropped_ims[i, :, :, :] = img
    cls_scores, reg = predict_onet(cropped_ims)
...
# 画出人脸框和关键点
def draw_face(image_path, boxes_c):
...
if __name__ == '__main__':
    # 预测图片获取人脸的box和关键点
    boxes_c = infer_image(args.image_path)
    print(boxes_c)
    # 把关键画出来
    if boxes_c is not None:
        draw_face(image_path=args.image_path, boxes_c=boxes_c)
    else:
