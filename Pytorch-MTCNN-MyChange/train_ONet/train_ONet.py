...
for epoch in range(epoch_num):
    for batch_id, (img, label, bbox) in enumerate(train_loader):
        img = img.to(device)
        ...
        if batch_id % 100 == 0:
            ...
            print('[%s] Train epoch %d, batch %d, total_loss: %f, cls_loss: %f, box_loss: %f, '
                  'accuracy：%f' % (
                      datetime.now(), epoch, batch_id, total_loss, cls_loss, box_loss, acc))
    scheduler.step()

    # 保存模型
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    torch.jit.save(torch.jit.script(model), os.path.join(model_path, 'my-ONet.pth'))
